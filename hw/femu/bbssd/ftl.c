...
static double  get_read_lat(struct ssd *ssd, int page_type, int age, struct ppa *ppa){
	int retry = 0;
	...
	if (page_age == 0) {
		...
		if (page_type == 1){
			...
			read_page_lat = QLC_U_1_R * retry;
		} else if (page_type == 6){
			read_page_lat = QLC_U_2_R * retry;
		} else {
			read_page_lat = QLC_U_3_R * retry;
		}
	} else if (page_age == 2) {
		if (page_type == 1){
			...
			retry = MID_READ_U_RETRY1;
			read_page_lat = QLC_U_1_R * retry;
		} else if (page_type == 6){
			retry = MID_READ_U_RETRY2;
			read_page_lat = QLC_U_2_R * retry;
		} else {
			retry = MID_READ_U_RETRY3;
			read_page_lat = QLC_U_3_R * retry;
		}
	} else {
		if (page_type == 1){
			...
			retry = OLD_READ_RETRY2;
			read_page_lat = QLC_2_R * retry;
		} else if (page_type == 5){
			retry = OLD_READ_U_RETRY1;
			read_page_lat = QLC_U_1_R * retry;
		} else if (page_type == 6){
			retry = OLD_READ_U_RETRY2;
			read_page_lat = QLC_U_2_R * retry;
		} else {
			retry = OLD_READ_U_RETRY3;
			read_page_lat = QLC_U_3_R * retry;
		}
	}
...
static inline void set_maptbl_ent(struct ssd *ssd, uint64_t lpn, struct ppa *ppa)
{
	...
	if (mapped_ppa(&back_ppa)) {
		struct ru *back_ru = get_ru(ssd, &back_ppa);
		// 找原始数据的加速价值
		struct ppa origin_ppa = get_maptbl_ent(ssd, lpn);
		int page_type = get_ppa_page_type(ssd, &origin_ppa);
		double read_page_lat = get_read_lat(ssd, page_type, ssd->age, &origin_ppa);

		int new_page_type = get_ppa_page_type(ssd, ppa);
		double new_read_page_lat = get_read_lat(ssd, new_page_type, ssd->age, ppa);

		back_ru->total_value -= ssd->read_hotness[lpn] * read_page_lat;
		back_ru->total_value += ssd->read_hotness[lpn] * new_read_page_lat;
	}
    ftl_assert(lpn < ssd->sp.tt_pgs);
    ssd->maptbl[lpn] = *ppa;
}
...
static void ssd_init_fdp_ru_mgmts(struct ssd *ssd)
{
	...
	for (int i = 0; i < nrg; i++) {
		...
		// 初始化三区大小
		ssd->high_area_ru_num = 0;
		ssd->change_area_ru_num = ssd->sp.tt_rus - ssd->high_area_ru_num;
...
static int get_next_free_ruid(struct ssd *ssd, struct fdp_ru_mgmt *rum, int ruhid)
{
	...
	// 可转换区第一阶段选最老的，其余的都是选最年轻的 TODO
	if (ssd->sp.wl_mode == 3) {

		// 判断优先取哪个
		double erase_ratio2 = ssd->total_high_area_erase * (ssd->rums_slc[0].tt_rus - ssd->high_area_ru_num) * ssd->sp.endurance_qlc / (ssd->high_area_ru_num * (ssd->erase_slc - ssd->total_high_area_erase) * ssd->sp.endurance_slc);
		if (erase_ratio2 > 1)
			ssd->page_allocate_mode = 1;
		else
			// 有限写入永久slc块
			ssd->page_allocate_mode = 0;

		struct ru *victim_mode0_ru = NULL;
		double victim_mode0_ru_erase = 10000;
		struct ru *victim_mode1_ru = NULL;
		double victim_mode1_ru_erase;
		if (ssd->wl_stage == 0)	{
			victim_mode1_ru_erase = -1;
		} else {
			victim_mode1_ru_erase = 10000;
		}

		// 待选块为永久SLC块
		if (ru_tmp->area_mode == 0) {
			victim_mode0_ru = ru_tmp;
			victim_mode0_ru_erase = ru_tmp->erase;
		} else {
			victim_mode0_ru = ru_tmp;
			victim_mode1_ru_erase = ru_tmp->erase;
		}
		...
		for (int i = 1; i < rum->free_ru_cnt; i++) {
			retru = QTAILQ_NEXT(retru, entry);
			// 最年轻的永久SLC块
			if (retru->area_mode == 0 && retru->erase < victim_mode0_ru_erase) {
				victim_mode0_ru_erase = retru->erase;
				victim_mode0_ru = retru;
			}
			if(ssd->wl_stage == 0) {
				if (retru->area_mode == 1 && retru->erase > victim_mode1_ru_erase) {
					victim_mode1_ru_erase = retru->erase;
					victim_mode1_ru = retru;
				}
			} else {
				if (retru->area_mode == 1 && retru->erase < victim_mode1_ru_erase) {
					victim_mode1_ru_erase = retru->erase;
		...
		if (ssd->page_allocate_mode == 0) {
			if (victim_mode0_ru)
				ru_tmp = victim_mode0_ru;
			else 
				ru_tmp = victim_mode1_ru;
		} else {
			if (victim_mode1_ru)
				ru_tmp = victim_mode1_ru;
			else 
...
static void ssd_init_params(struct ssdparams *spp, FemuCtrl *n)
{
	...
	spp->enable_cap_loss = n->bb_params.enable_cap_loss;
...
static inline bool if_satisfy_migrate(struct ssd *ssd, struct ppa *ppa) {
	...
	if (migrate_mode == 2) {
		...
		if (ssd->read_hotness[lpn] >= 2 && (page_type == 7 || page_type == 8))
			return true;
		return false;
	}
	...
	if (migrate_mode == 3) {
		int page_type = get_ppa_page_type(ssd, ppa);
		double read_page_lat = get_read_lat(ssd, page_type, ssd->age, ppa);

		// 被迁移的页热度
		double cur_hotness = ssd->read_hotness[lpn] * read_page_lat;
		double hotless = 0;
		struct ru *hotless_ru =  select_ra_victim_ru(ssd);
		if (hotless_ru != NULL) {
			hotless = hotless_ru->total_value;
		}
		double cur_slc_avg_hotness = hotless * 4.0 / ssd->sp.pgs_per_ru;

		// if ((cur_hotness >= cur_slc_avg_hotness + SLC_W) || (cur_hotness >= SLC_W && ssd->sp.ra_max_cnt > ssd->rums_slc[0].read_ru_cnt + 1)){
			// ftl_log("read_hotness[cur_lpn]:%lf %lf\n", ssd->read_hotness[lpn], cur_hotness);
		// if (cur_hotness >= cur_slc_avg_hotness + SLC_W && ssd->read_hotness[lpn] >= 3) {
		if (cur_hotness >= cur_slc_avg_hotness + SLC_W && ssd->read_hotness[lpn] >= 2) {
			return true;
		}
		else
			return false;
        ...
        nand_cmd *ncmd)
{
	...
	if (c == NAND_SLC_READ || c == NAND_QLC_READ){
		...
		int read_retry = 0;
		if(page_age == 2 && c == NAND_QLC_READ) {
			int page_type = get_ppa_page_type(ssd, ppa);
			if (page_type == 1) {
				operation_lat = NAND_QLC_READ_L_LAT * MID_READ_RETRY1;
				read_retry = MID_READ_RETRY1 - 1;
			}
			else if (page_type == 2) {
				operation_lat = NAND_QLC_READ_CL_LAT * MID_READ_RETRY2;
				read_retry = MID_READ_RETRY2 - 1;
			}
			else if (page_type == 3) {
				operation_lat = NAND_QLC_READ_CU_LAT * MID_READ_RETRY2;
				read_retry = MID_READ_RETRY2 - 1;
			}
			else if (page_type == 4) {
				operation_lat = NAND_QLC_READ_U_LAT * MID_READ_RETRY2;
				read_retry = MID_READ_RETRY2 - 1;
			} else if (page_type == 5) {
				operation_lat = NAND_QLC_READ_L_LAT * MID_READ_U_RETRY1;
				read_retry = MID_READ_U_RETRY1 - 1;
			}
			else if (page_type == 6) {
				operation_lat = NAND_QLC_READ_CL_LAT * MID_READ_U_RETRY2;
				read_retry = MID_READ_U_RETRY2 - 1;
			}
			else if (page_type == 7) {
				operation_lat = NAND_QLC_READ_CU_LAT * MID_READ_U_RETRY3;
				read_retry = MID_READ_U_RETRY3 - 1;
			}
			else {
				operation_lat = NAND_QLC_READ_U_LAT * MID_READ_U_RETRY3;
				read_retry = MID_READ_U_RETRY3 - 1;
			}
		}

		if(page_age == 3 && c == NAND_QLC_READ) {
			int page_type = get_ppa_page_type(ssd, ppa);
			if (page_type == 1) {
				operation_lat = NAND_QLC_READ_L_LAT * OLD_READ_RETRY1;
				read_retry = OLD_READ_RETRY1 - 1;
			}
			else if (page_type == 2) {
				operation_lat = NAND_QLC_READ_CL_LAT * OLD_READ_RETRY2;
				read_retry = OLD_READ_RETRY2 - 1;
			}
			else if (page_type == 3) {
				operation_lat = NAND_QLC_READ_CU_LAT * OLD_READ_RETRY2;
				read_retry = OLD_READ_RETRY2 - 1;
			}
			else if (page_type == 4) {
				operation_lat = NAND_QLC_READ_U_LAT * OLD_READ_RETRY2;
				read_retry = OLD_READ_RETRY2 - 1;
			} else if (page_type == 5) {
				operation_lat = NAND_QLC_READ_L_LAT * OLD_READ_U_RETRY1;
				read_retry = OLD_READ_U_RETRY1 - 1;
			}
			else if (page_type == 6) {
				operation_lat = NAND_QLC_READ_CL_LAT * OLD_READ_U_RETRY2;
				read_retry = OLD_READ_U_RETRY2 - 1;
			}
			else if (page_type == 7) {
				operation_lat = NAND_QLC_READ_CU_LAT * OLD_READ_U_RETRY3;
				read_retry = OLD_READ_U_RETRY3 - 1;
			}
			else {
				operation_lat = NAND_QLC_READ_U_LAT * OLD_READ_U_RETRY3;
				read_retry = OLD_READ_U_RETRY3 - 1;
			}
		}

		if (page_age == 3 && c == NAND_SLC_READ) {
			operation_lat = NAND_SLC_READ_LAT * 2;
			read_retry = 1;
		}
		spp->read_retry += read_retry;
		if (ncmd->type == WL_IO) {
			spp->read_retry -= read_retry;
			operation_lat = operation_lat * 1.0/ spp->gap;
		}
...
static void mark_page_invalid(struct ssd *ssd, struct ppa *ppa, uint16_t rgid)
{
	...
	if (ru->ruhid == 1) {
		...
		int page_type = get_ppa_page_type(ssd, &origin_ppa);
		double read_page_lat = get_read_lat(ssd, page_type, ssd->age, &origin_ppa);
		ru->total_value -= ssd->read_hotness[lpn] * read_page_lat;
	}
...
static uint64_t fdp_gc_write_page(struct ssd *ssd, struct ppa *old_ppa, uint16_t rgid, uint16_t ruhid)
{
	...
	if ((ssd->sp.write_mode == 1 || ssd->sp.write_mode == 5) && ssd->sp.read_migration == 3 && ruhid == 2) {
		int page_type = get_ppa_page_type(ssd, &new_ppa);
		double read_page_lat = get_read_lat(ssd, page_type, ssd->age,  &new_ppa);
		...
		if (cur_hotness >= cur_slc_avg_hotness + SLC_W && ssd->read_hotness[lpn] >= 2) {
			...
			set_rmap_ent(ssd, lpn, &ppa);
			mark_page_valid(ssd, &ppa);
			// ftl_log("migrate_count2:%"PRIu64"\n", ssd->migrate_count);
			ssd_advance_fdp_write_pointer(ssd, 0, 0, 1, false, 0);
...
static struct ru *select_ra_victim_ru(struct ssd *ssd) {
	...
	for (int j = 0; j < rum->victim_ru_cnt; j++) {
		...
		if (tmp_ru->ruhid == 1) {
			if (tmp_ru->total_value < ra_victim_ru_hotness) {
				ra_victim_ru = tmp_ru;
				ra_victim_ru_hotness = tmp_ru->total_value;
			}
		}
	}
	// 在full list 中寻找热度最低的
	...
	if (ra_full_ru != NULL) {
		if (ra_full_ru->total_value < ra_victim_ru_hotness && ra_full_ru->ruhid == 1) {
			ra_victim_ru = ra_full_ru;
			ra_victim_ru_hotness = ra_full_ru->total_value;
		}
	}
...
static struct ru *select_wa_victim_ru(struct ssd *ssd) {
	...
	if (ssd->sp.wl_mode == 3) {
		...
		for (int j = 0; j < rum->victim_ru_cnt; j++) {
			...
			if (tmp_ru->area_mode == 0)
				victim_score = tmp_ru->ipc * 1.0 / ssd->sp.pgs_per_ru;
			else if (ssd->wl_stage == 0)
				victim_score = 0.8 * tmp_ru->ipc / ssd->sp.pgs_per_ru + 0.2 * tmp_ru->erase / ssd->sp.endurance_qlc;
			else 
				victim_score = 0.8 * tmp_ru->ipc / ssd->sp.pgs_per_ru + 0.2 * (ssd->sp.endurance_qlc - tmp_ru->erase) / ssd->sp.endurance_qlc;
			if (tmp_ru->ruhid == 0) {
				if (victim_score > max_score) {
					max_score = victim_score;
					wa_victim_ru = tmp_ru;
				}
			}
		}
...
static struct ru *select_victim_ru_slc(struct ssd *ssd, bool force, int rgid)
{
	...
	if (ssd->sp.dynamic_ra_flag && ssd->ra_full_flag == 1 && ssd->sp.wa_max_cnt > 10 && victim_ra_ru != NULL && (victim_ra_ru->total_value > value_thre_high || ssd->rums_slc[0].write_ru_cnt < ssd->sp.wa_max_cnt - 1)) {
		ssd->sp.ra_max_cnt ++;
		ssd->sp.wa_max_cnt --;
		if (ssd->rums_slc[0].read_ru_cnt >= ssd->sp.ra_max_cnt)
			ssd->ra_full_flag = 1;
		else
			ssd->ra_full_flag = 0;
		if (ssd->rums_slc[0].write_ru_cnt >= ssd->sp.wa_max_cnt)
			ssd->wa_full_flag = 1;
		else
			ssd->wa_full_flag = 0;
		ssd->observe_flag = 1;
	...
	// 判断是否要扩大写区
	if (ssd->sp.dynamic_ra_flag && ssd->wa_full_flag == 1 && ssd->sp.ra_max_cnt > 3 && victim_ra_ru != NULL && victim_ra_ru->total_value < value_thre_low) {
		ssd->sp.ra_max_cnt --;
		ssd->sp.wa_max_cnt ++;
		if (ssd->rums_slc[0].read_ru_cnt >= ssd->sp.ra_max_cnt)
			ssd->ra_full_flag = 1;
...
static int fdp_clean_one_block(struct ssd *ssd, struct ppa *ppa, uint16_t rgid, uint16_t ruhid, int no_record_flag, double cur_slc_avg_hotness)
{
    struct ssdparams *spp = &ssd->sp;
    ...
    for (int pg = 0; pg < spp->pgs_per_blk; ) {
        ...
        if (pg_iter->status == PG_VALID) {
			...
			if (ruhid != 1) {
				...
				if ( mode == 0) {
					if (ssd->sp.write_mode == 1 || ssd->sp.write_mode == 5) {
						if (ssd->gc_cnt_before_update[cur_lpn] >= ssd->gc_cnt_before_update_thre[ssd->write_hotness[cur_lpn]]) {
							...
							} else if (ssd->gc_cnt_before_update_thre[ssd->write_hotness[cur_lpn]] == 1) {
								ssd->cnt_23[ssd->write_hotness[cur_lpn]] ++;
							} else if (ssd->gc_cnt_before_update_thre[ssd->write_hotness[cur_lpn]] == 0) {
				...
				if ((ssd->sp.write_mode == 1 || ssd->sp.write_mode == 5) && ssd->sp.read_migration == 3) {
					struct ppa origin_ppa = get_maptbl_ent(ssd, cur_lpn);
					int page_type = get_ppa_page_type(ssd, &origin_ppa);
					double read_page_lat = get_read_lat(ssd, page_type, ssd->age, &origin_ppa);

					// 被迁移的页热度
					double cur_hotness = ssd->read_hotness[cur_lpn] * read_page_lat;
					// if ((cur_hotness >= cur_slc_avg_hotness + SLC_W) || (cur_hotness >= SLC_W && ssd->sp.ra_max_cnt > ssd->rums_slc[0].read_ru_cnt + 1)) {
						// ftl_log("cur_slc_avg_hotness:%lf %lf\n", cur_slc_avg_hotness, cur_hotness);
					// if (cur_hotness >= cur_slc_avg_hotness + SLC_W && ssd->read_hotness[cur_lpn] >= 3) {
					if (cur_hotness >= cur_slc_avg_hotness + SLC_W && ssd->read_hotness[cur_lpn] >= 2) {
						ssd->migrate_count ++;
						spp->pages_from_migrate ++;
						ssd->cur_write_req_cnt ++;
						...
						/* update maptbl */
						// 不去除原有映射，而是加上备份映射
						set_back_maptbl_ent(ssd, cur_lpn, &back_ppa);
						set_rmap_ent(ssd, cur_lpn, &back_ppa);
						mark_page_valid(ssd, &back_ppa);
						// ftl_log("migrate_count1:%"PRIu64"\n", ssd->migrate_count);
						ssd_advance_fdp_write_pointer(ssd, 0, 0, 1, true, 0);
						...
						swr.stime = 0;
						ssd_advance_status(ssd, &back_ppa, &swr);
					}
				}
			}
...
static void output_info_log(struct ssd *ssd) {
	...
	fprintf(fp_ec_info, "total:%d %d %lf %lf\n", ssd->high_area_ru_num, ssd->change_area_ru_num, ssd->total_high_area_erase / ssd->high_area_ru_num, ssd->total_change_area_erase / ssd->change_area_ru_num);
	fclose(fp_ec_info);
	...
	double wl_divide_gap = ssd->sp.pages_from_wl * 1.0 / ssd->sp.gap;
	double wa = (wl_divide_gap + (ssd->sp).pages_from_gc + (ssd->sp).pages_from_host + (ssd->sp).pages_from_migrate) * 1.0 / ((ssd->sp).pages_from_host);
	fprintf(fp_wara, "pages_from_wl:%lf pages_from_gc:%"PRIu64" pages_from_host:%"PRIu64" pages_from_migrate:%"PRIu64"\n", wl_divide_gap, (ssd->sp).pages_from_gc, (ssd->sp).pages_from_host, (ssd->sp).pages_from_migrate);
	double ra = ((ssd->sp).pages_from_host_read + (ssd->sp).read_retry + (ssd->sp).pages_from_gc) * 1.0 / ((ssd->sp).pages_from_host_read);
	fprintf(fp_wara, "wa:%lf ra:%lf migrate_count:%"PRIu64" write_migrate_count:%"PRIu64" slc_wc:%"PRIu64" qlc_wc:%"PRIu64" read_cnt:%d %d %d %d %d %d %d %d erase:%d %d %lf %lf hit:%d %d\n", wa, ra, ssd->migrate_count, ssd->write_migrate_count, ssd->rums_slc[0].write_cnt, ssd->rums_qlc[0].write_cnt, ssd->read_cnt[0], ssd->read_cnt[1], ssd->read_cnt[2], ssd->read_cnt[3], ssd->read_cnt[4], ssd->read_cnt[5], ssd->read_cnt[6],ssd->read_cnt[7], ssd->high_area_ru_num, ssd->change_area_ru_num, ssd->total_high_area_erase / ssd->high_area_ru_num, ssd->total_change_area_erase / ssd->change_area_ru_num, ssd->total_buffer_count, ssd->hit_buffer_count);
	fclose(fp_wara);
	return;
}
...
static void erase_victim_ru(struct ssd *ssd, int victim_ru_id, int mode,  uint16_t rgid, int no_record_flag) { 
	...
	ftl_log("GC-ing ru:%d,ipc=%d,vpc=%d,r_hotness:%lf, victim=%d,full=%d,free=%d,bad=%d,read=%d,write=%d,ruhid=%d\n", victim_ru->id,
              victim_ru->ipc, victim_ru->vpc, ru_read_hotness, rum->victim_ru_cnt, rum->full_ru_cnt, rum->free_ru_cnt, rum->bad_ru_cnt, rum->read_ru_cnt, rum->write_ru_cnt, ruhid);

	ssd->slc_gc_eff = victim_ru->ipc * 1.0 / ssd->sp.pgs_per_ru;
	double hotness = 0;
	if (victim_ru->ruhid == 1) {
		hotness = victim_ru->total_value * 4.0 / victim_ru->vpc;
	}
	...
	if (ssd->cur_write_req_cnt >= (10 * ssd->sp.pgs_per_ru / 4)  && (ssd->sp.write_mode == 1 || ssd->sp.write_mode == 5)) {
		...
		if (page_age == 3)
			retry_slc = 2;		
		double c_before = ssd->action_1_cnt * SLC_W + ssd->action_2_cnt * (SLC_W + SLC_R * retry_slc) + ssd->action_3_cnt * (SLC_R * retry_slc + QLC_W);
		...
		if (erase_ratio >= 0.9 && erase_ratio <= 1.1)
			ssd->wfc_flag = 0;
		...
		if (ssd->sp.wl_mode == 2 && erase_ratio > 1.1) {
			ssd->write_hotness_thre = ssd->write_hotness_thre >= 4 ? 4 : ssd->write_hotness_thre + 1;
			ssd->wfc_flag = 1;
		} else if (ssd->sp.wl_mode == 2 && erase_ratio < 0.9) {
		...
		if (ssd->sp.dynamic_tm_flag == 1) {
			...
			for (int j = 0;	j < 4; j ++) {
				...
				double b_before;
				double b_after;
				double ref_ratio = 1.0 * (QLC_W - SLC_W) / (SLC_W + SLC_R * retry_slc); 
				// if (ssd->gc_cnt_before_update_thre[j] == 3) {
				// 	b_before = ssd->cnt_41[j] * (QLC_W - SLC_W - 3 * (SLC_W + SLC_R * retry_slc)) + ssd->cnt_45[j] * (0 - 4 * (SLC_W + SLC_R * retry_slc));
				// 	b_after = (ssd->cnt_41[j] + ssd->cnt_45[j]) * (0 - 3 * (SLC_W + SLC_R * retry_slc));
				// } else if (ssd->gc_cnt_before_update_thre[j] == 2) {
				// 	b_before = ssd->cnt_31[j] * (QLC_W - SLC_W - 2 * (SLC_W + SLC_R * retry_slc)) + ssd->cnt_34[j] * (0 - 3 * (SLC_W + SLC_R * retry_slc));
				// 	b_after = (ssd->cnt_31[j] + ssd->cnt_34[j]) * (0 - 2 * (SLC_W + SLC_R * retry_slc));
				// } else if (ssd->gc_cnt_before_update_thre[j] == 1) {
				// 	b_before = ssd->cnt_21[j] * (QLC_W - SLC_W - 1 * (SLC_W + SLC_R * retry_slc)) + ssd->cnt_23[j] * (0 - 2 * (SLC_W + SLC_R * retry_slc));
				// 	b_after = (ssd->cnt_21[j] + ssd->cnt_23[j]) * (0 - 1 * (SLC_W + SLC_R * retry_slc));
				// } else if (ssd->gc_cnt_before_update_thre[j] == 0) {
				// 	b_before = ssd->cnt_11[j] * (QLC_W - SLC_W) + ssd->cnt_12[j] * (0 - 1 * (SLC_W + SLC_R * retry_slc));
				// 	b_after = 0;
				// } else {
				// 	b_before = 0;
				// 	b_after = 0;
				...
				if (ssd->gc_cnt_before_update_thre[j] == 3) {
					b_before = ssd->cnt_45[j];
					b_after = ssd->cnt_41[j];
				} else if (ssd->gc_cnt_before_update_thre[j] == 2) {
					b_before = ssd->cnt_34[j];
					b_after = ssd->cnt_31[j];
				} else if (ssd->gc_cnt_before_update_thre[j] == 1) {
					b_before = ssd->cnt_23[j];
					b_after = ssd->cnt_21[j];
				} else if (ssd->gc_cnt_before_update_thre[j] == 0) {
					b_before = ssd->cnt_12[j];
					b_after = ssd->cnt_11[j];
				} else {
					b_before = 0;
					b_after = 0;
				}
	...
	if (mode == 0) {
		if (!no_record_flag) {
			...
			ssd->erase_slc += spp->slc_alpha * spp->gap;
		}
	...
	victim_ru->erase = spp->slc_alpha * victim_ru->pe_slc + victim_ru->pe_qlc;
	...
	if (victim_ru->erase > ssd->sp.endurance_qlc && victim_ru->area_mode != 0) {
		ssd->exceed_qlc_block_num ++;
		ftl_log("Ru %d %lfbecomes forever slc!\n", victim_ru->id, victim_ru->erase);
		// // 将该块的信息与SLC区域中信息交换以模拟该块被放置在SLC区域
		// int min_index = -1;
		// int min_value = 100000;
		// for (int i = 0; i < ssd->rums_slc[0].tt_rus; i++) {
		// 	int index = get_slc_ru_id(ssd, i);
		// 	if (ssd->rus[index].area_mode == 1 && ssd->rus[index].erase < min_value) {
		// 		min_index = index;
		// 		min_value = ssd->rus[index].erase;
		// 	}
		// }
		// double victim_erase = victim_ru->erase;
		// int victim_peslc = victim_ru->pe_slc;
		// int victim_peqlc = victim_ru->pe_qlc;
		// if (min_index != -1) {
		// 	victim_ru->erase = ssd->rus[min_index].erase;
		// 	victim_ru->pe_slc = ssd->rus[min_index].pe_slc;
		// 	victim_ru->pe_qlc = ssd->rus[min_index].pe_qlc;

		// 	ssd->rus[min_index].area_mode = 0;
		// 	ssd->rus[min_index].pe_slc = victim_peslc;
		// 	ssd->rus[min_index].pe_qlc = victim_peqlc;
		// 	ssd->rus[min_index].erase = victim_erase;
		// }
		...
		victim_ru->area_mode = 0;
		if (ssd->exceed_qlc_block_num >= 0.8 * (double)ssd->rums_slc[0].tt_rus) {
			ftl_log("wl_stage become 1\n");
			ssd->wl_stage = 1;
		}
	}

	victim_ru->total_value = 0;
    ...
    // 块到达QLC的磨损上限，永久作为SLC使用
    if (victim_ru->erase >= victim_ru->rand_rate * (double) spp->endurance_slc) {
		ssd->exceed_qlc_block_num --;
		ssd->exceed_slc_block_num ++;
		QTAILQ_INSERT_TAIL(&rum->bad_ru_list, victim_ru, entry);
		rum->bad_ru_cnt++;
		ftl_log("Ru %d becomes bad!\n", victim_ru->id);
	} 
	double cap_loss = 3.0 * ssd->exceed_qlc_block_num / 4 + ssd->exceed_slc_block_num;
	if (cap_loss >= ssd->sp.enable_cap_loss){
		output_info_log(ssd);
		ftl_err("SSD reaches its end of the life!\n");
		abort();
	} else {
		...
		if (ssd->sp.enable_swl == 1 && (ssd->sp.wl_mode == 0 || ssd->sp.wl_mode == 1 || ssd->sp.wl_mode == 2)) {
			...
			if (youngest_ru_id != -1 && youngest_ru_id != victim_ru->id) { 
				double threshold = erase_sum / wl_rus / 2 + endurance / 2; // 交换阈值（热ru磨损次数大于平均磨损次数+...）
				if (victim_ru->erase > threshold) {
					...
					if (mode == 0) {
						ssd->rus[youngest_ru_id].pe_slc += 1;
						ssd->erase_slc += spp->slc_alpha;
					} else {
						ssd->rus[youngest_ru_id].pe_qlc += 1;
						ssd->erase_qlc += 1;
					}
					
					ssd->total_change_area_erase -= ssd->rus[youngest_ru_id].erase;
					youngest_ru_erase = ssd->rus[youngest_ru_id].pe_slc * spp->slc_alpha + ssd->rus[youngest_ru_id].pe_qlc;
					ssd->total_change_area_erase += youngest_ru_erase;
					int youngest_ru_slc = ssd->rus[youngest_ru_id].pe_slc;
					int youngest_ru_qlc = ssd->rus[youngest_ru_id].pe_qlc;


					ftl_log("static WL complete between ru %d (ec = %lf) and ru %d (ec = %lf)\n", victim_ru->id, victim_ru->erase, youngest_ru_id, youngest_ru_erase);
					...
					victim_ru->pe_slc = youngest_ru_slc;
					victim_ru->pe_qlc = youngest_ru_qlc;
				}
			}
		}

		// 受害块得能参与动态转化
		// 当前SLC区域磨损程度过高时执行全局磨损均衡
		if ((ssd->sp.wl_mode == 0  && victim_ru->erase < spp->endurance_qlc && victim_ru->mode == 0) || (ssd->sp.wl_mode == 3 && victim_ru->erase < spp->endurance_qlc && victim_ru->mode == 0 && erase_ratio > 1)){
			int youngest_ru_id = -1;
			double youngest_ru_erase = 1000000;
			double erase_sum = 0; // 磨损总次数
			int wl_rus = 0; // 磨损ru数量
			...
			for (int i = 0; i < ssd->sp.tt_rus; i++) {
				...
				// 找到转换区内磨损最小的ru
				if (cur_ru->erase < youngest_ru_erase && cur_ru->mode == 1) {
					youngest_ru_id = cur_ru->id;
					youngest_ru_erase = cur_ru->erase;
				}
			}
			...
			if (youngest_ru_id != -1 && youngest_ru_id != victim_ru->id) { 
				double threshold = erase_sum / wl_rus / 2 + spp->endurance_qlc / 2; // 交换阈值
				if (victim_ru->erase > threshold) {
					...
					ssd->rus[youngest_ru_id].pe_qlc = victim_ru->pe_qlc;
					ssd->rus[youngest_ru_id].pe_slc = victim_ru->pe_slc;
					ftl_log("before wl:%lf %lf %lf\n", ssd->erase_slc, ssd->erase_qlc, erase_ratio);
					ssd->erase_slc -= victim_ru->erase;
					ssd->erase_qlc += victim_ru->erase;
					//ssd->rus[youngest_ru_id].mode = 1;
					victim_ru->erase = youngest_ru_erase;
					victim_ru->pe_slc = youngest_ru_peslc;
					victim_ru->pe_qlc = youngest_ru_peqlc;
					ssd->erase_slc += youngest_ru_erase;
					ssd->erase_qlc -= youngest_ru_erase;
					erase_ratio = ssd->erase_slc * ssd->sp.endurance_qlc * ssd->sp.qlc_op / (ssd->erase_qlc * ssd->sp.endurance_slc * ssd->sp.slc_op);
					ftl_log("after wl:%lf %lf %lf\n", ssd->erase_slc, ssd->erase_qlc, erase_ratio);
					//victim_ru->mode = 0;
				}
			}
		}
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
		...
		if (mapped_ppa(&back_ppa)) {
			...
			int page_type = get_ppa_page_type(ssd, &origin_ppa);
			double read_page_lat = get_read_lat(ssd, page_type, ssd->age, &origin_ppa);
			back_up_ru->total_value = back_up_ru->total_value + (ssd->read_hotness[lpn] - pre_read_hotness) * read_page_lat;
				...
				if (spp->write_mode == 1 || spp->write_mode == 5) {
					if (ssd->gc_cnt_before_update_thre[ssd->write_hotness[lpn]] == 0) {
						...
						if (ssd->gc_cnt_before_update[lpn] >= 1)
							...
							ssd->cnt_11[ssd->write_hotness[lpn]] ++;
					} else if (ssd->gc_cnt_before_update_thre[ssd->write_hotness[lpn]] == 2) {
						if (ssd->gc_cnt_before_update[lpn] >= 2)
							ssd->cnt_31[ssd->write_hotness[lpn]] ++;
						else if  (ssd->gc_cnt_before_update[lpn] == 1)
							ssd->cnt_21[ssd->write_hotness[lpn]] ++;
						else
							ssd->cnt_11[ssd->write_hotness[lpn]] ++;
					} else if (ssd->gc_cnt_before_update_thre[ssd->write_hotness[lpn]] == 3) {
						if (ssd->gc_cnt_before_update[lpn] >= 3)
    ...
    while (1) {
		...
		if (cur_time - start_time2 >= time_gap2) {
			...
			for (uint64_t j = 0; j < ssd->sp.tt_pgs; j++) {
				...
				struct ppa back_ppa = get_back_maptbl_ent(ssd, j);
				if (mapped_ppa(&back_ppa)) {
					ru = get_ru(ssd, &back_ppa);
					if (ru->ruhid == 1) {
						int page_type = get_ppa_page_type(ssd, &ppa);
						double read_page_lat = get_read_lat(ssd, page_type, ssd->age, &ppa);
						ru->total_value += ssd->read_hotness[j] * read_page_lat;
					}
				}
